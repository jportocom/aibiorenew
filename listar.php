...
<head>
    ...
    <title>Todas as Medições — Porto Metabolic Health</title>
    ...
    <style>
        ...
        @media (max-width: 640px) {
            .grafico-header { flex-direction: column; align-items: stretch; }
            .opcoes-vista { flex-direction: column; align-items: flex-start; border-radius: var(--r); }
...
<div class="page-hero">
    ...
    <h1>Todas as Medições</h1>
    <p class="lead">Histórico completo das suas medições, por ordem cronológica decrescente.</p>
</div>

<section>
    <div class="wrap">
        <div style="display:flex; justify-content:flex-end; margin-bottom:var(--s5);">
            <a href="/dev/medepeso/inserir.php" class="btn btn-primary">+ Inserir Medição</a>
        </div>
        <div id="areaTabela">
            <div class="vazio">A carregar…</div>
        </div>

        <div class="grafico-secao" id="areaGrafico" style="display:none;">
            <div class="grafico-header">
                <h2>Evolução do Peso</h2>
                <div class="opcoes-vista" id="opcoesPainel">
                    <label>
            ...
            <div style="display:flex; justify-content:flex-end; margin-bottom:var(--s3);">
                <a href="/dev/medepeso/grafico.php" class="btn" style="font-size:.82rem; padding:var(--s2) var(--s4); background:var(--fundo); border:1px solid var(--borda); color:var(--texto-2); border-radius:var(--r); text-decoration:none;">
                    &#9974; Ver gráfico em ecrã inteiro
                </a>
            </div>
...
function pesoNaData(medicoes, dataISO) {
    // medicoes em ordem DESC (mais recente primeiro)
...
function verificarViabilidade(medicoes, objetivo) {
    ...
    var maxPerda = Math.round(dias * 10) / 100;   // dias × 0,1 kg
...
function construirGrafico(opcao) {
    ...
    // ── OPÇÃO C: só projecção com sextas-feiras ──────────────────────
    if (opcao === 'C') {
        if (!objetivo) {
            msgSemObj.innerHTML = 'Opção C requer um objectivo definido em ' +
    ...
    // Data e peso de início da linha de projecção (usa data_ini_objetiv se disponível)
    ...
    /* Opção B: prolongar a linha verde até à data de início do objectivo */
    if (mostrarObjetivo && objetivo) {
    ...
    if (mostrarObjetivo && !objetivo) {
        msgSemObj.innerHTML = 'Opção B requer um objectivo definido em ' +
            '<a href="/dev/medepeso/gestaodaapp.php" style="color:var(--verde);">Gestão de App</a>.';
        msgSemObj.style.display = 'block';
    }
}
...
async function carregarMedicoes() {
    ...
    try {
        ...
        if (!dMedicoes.sucesso) {
            area.innerHTML = '<div class="vazio">Erro ao carregar medições.</div>';
            return;
        }
        if (dMedicoes.medicoes.length === 0) {
            area.innerHTML = '<div class="vazio">Ainda não há medições registadas. <a href="/dev/medepeso/inserir.php" style="color:var(--verde)">Inserir a primeira</a>.</div>';
            return;
        }
        ...
        dMedicoes.medicoes.forEach(function(m) {
            html += '<tr>'
